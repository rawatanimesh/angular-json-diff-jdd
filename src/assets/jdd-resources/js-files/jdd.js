...
var jdd = {
  ...
  config1List: [],
  config2List: [],
  config1CurrentIndex: 0,
  config2CurrentIndex: 0,
  isActive: false,
  preNumberOfLines: 500,
  blockHeight: 0,
  /**
  ...
  formatPRETags: function () {
    forEach($('pre'), function (pre) {
      ...
      var addLine = function (line, index) {
        if (linesLength > 1) {
          index =
            jdd.config1CurrentIndex != 0
              ? index + jdd.preNumberOfLines * jdd.config1CurrentIndex
              : index;
  ...
  chunkString: function (str, len) {
    ...
    // const size = dataJSOn/len
    var r = Array(size);
    var offset = 0;
    var lend = len;
    for (let i = 0; i < size; i++) {
      // console.log("dataJSOn :",dataJSOn);
      // console.log("offset :",offset,"len :",lend);
      r[i] = dataJSOn.slice(offset, lend);
      if (offset == 0) {
        offset = 1 + len;
        lend = offset + len;
        // console.log("if :",offset);
      } else {
  ...
  compare: function () {
    ...
    //console.log('jdd.requestCount :', jdd.requestCount);
  ...
  /*
   * scroll function to load limited lines on scroll event
   */
  scroll: function () {
    ...
    if (jdd.config1List.length > 1 || jdd.config2List.length > 1) {
      $('#preBlock').bind('scroll', function () {
        if (!jdd.isActive) {
          jdd.isActive = true;
          /*
           * throttleHandler to controll scroll events
           */
          var throttleHandler = setTimeout(() => {
            jdd.isActive = false;
            //for scroll top
            if ($(this).scrollTop() == 0) {
              jdd.scrollTop();
              ...
              jdd.blockHeight = $(this).innerHeight();
              jdd.scrollBottom();
            }
          }, 400);
        }
  ...
  /*
   * scroll bottom funtion called at scroll function
   */
  ...
  /*
   * scroll top funtion called at scroll function
   */
  scrollTop: function () {
    if (jdd.config1CurrentIndex != 0) {
      jdd.config1CurrentIndex -= 1;
    }
  ...
  /*
   * jdd redefined functions for pre tag decoration
   */
  preDefinedJSONDiffFunctions: function () {
    jdd.formatPRETags();
    jdd.processDiffs();
    jdd.generateReport();
    if (jdd.diffs.length > 0) {
